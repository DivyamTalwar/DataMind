...
\onecolumn
\begin{center}
\includegraphics[width=0.84\textwidth]{figures/datamind_overview_v2.pdf}
\captionof{figure}{DataMind serving path. The Serving Agent routes a request
over profile-scoped document, database, graph, memory, and skill surfaces and
...
\begin{center}
\includegraphics[width=0.84\textwidth]{figures/datamind_build_agent_generated.png}
\captionof{figure}{DataMind-Build ingestion path. The Build Agent inventories
a profile's workspace, plans surface candidates, parses and normalizes sources,
projects records, validates candidate revisions, and publishes a profile
snapshot. The candidate descriptor remains unpublished until validation; reads
of touched surfaces are blocked during candidate assembly, and providers that
do not support historical artifacts fail closed if a pinned request becomes
stale.}
...
\begin{itemize}
  ...
  \item \textbf{RQ3}: What latency, throughput, and memory overhead does DataMind
  add under concurrent serving?
...
\subsection{RQ3: Runtime overhead and concurrency}
\label{sec:eval-rq3}

RQ3 measures the system cost of DataMind's serving path. We run the same
representative workload with the same model, prompt, tool schemas, and
surface contents under two conditions. \emph{DataMind no-hooks} disables the
PathAllowlist, destructive-SQL, and audit hooks while retaining the serving
runtime. \emph{DataMind full} uses the default configuration, including hooks,
evidence, snapshot metadata, and audit logging. If an external DB-GPT
implementation can be reproduced with the same model and data, we report it as
an additional reference; otherwise we do not attribute differences to an
unmatched external system.
...
latency, QPS, peak RSS, tool calls, input/output tokens, and a model-versus-tool
latency breakdown. QPS is completed tasks divided by wall-clock time. Error rate
is reported separately from latency averages. The comparison quantifies the
runtime cost of DataMind's policy, audit, evidence, and version metadata while
holding model behavior constant.
...
\centering\footnotesize
\setlength{\tabcolsep}{6pt}
\caption{RQ4 recovery after injected surface and build faults.}
\label{tab:rq4-faults}
\resizebox{0.94\textwidth}{!}{%
\begin{tabular}{@{}llrrrrrr@{}}
\toprule
Fault & System & Completion & Answer & Auditable & Structured error & Fallback/block & Recovery ms \\
\midrule
Parser/MinerU failure & Retry-only & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Parser/MinerU failure & DataMind-full & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Database/SQL timeout & Retry-only & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Database/SQL timeout & DataMind-full & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Active candidate read & Retry-only & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Active candidate read & DataMind-full & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Graph unavailable & Retry-only & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
