...
\subsection{RQ3: Updates and concurrency}
\label{sec:eval-rq3}
...
during, and after publication. We evaluate the current DataMind path as a
black-box system: reads of surfaces touched by an active candidate must be
blocked, publication must make the new logical revision visible, repeated writes
must be idempotent, and profiles must remain isolated. Snapshot identifiers and
receipts are retained in traces for diagnosis, but are not treated as separate
quality conditions.

Concurrent runs use 1, 5, 20, 50, and 100 sessions. We vary same-profile reads,
cross-profile reads, concurrent writes, and mixed read/write workloads. We report
blocked-read correctness, stale and mixed-snapshot outcomes, duplicate writes,
lost updates, cross-profile leakage, throughput, and p50/p95/p99 latency.
