...
\begin{table}[t]
\centering\footnotesize
\caption{Optional ingestion component sensitivity inherited from the earlier
report. The table is filled only when the same corpus and scorer are used.}
\label{tab:rq2-ingest}
\begin{tabular}{@{}lllrrr@{}}
\toprule
System & Chunker & Embedder & NDCG@10$\uparrow$ & Recall@10$\uparrow$ & MRR$\uparrow$ \\
\midrule
DataMind auto & built-in & \tbd & \tbd & \tbd & \tbd \\
DataMind BYOP & sentence & \tbd & \tbd & \tbd & \tbd \\
DataMind BYOP & sentence & \tbd & \tbd & \tbd & \tbd \\
DataMind BYOP & sentence & \tbd & \tbd & \tbd & \tbd \\
DB-GPT & built-in & \tbd & \tbd & \tbd & \tbd \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
DataMind no-hooks & 1 & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
DataMind full & 1 & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
...
DB-GPT (optional) & 1 & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[t]
\centering\footnotesize
\caption{Profile-scoped memory isolation diagnostic.}
...
\midrule
Flat namespace & \tbd & \tbd & \tbd & \tbd & \tbd \\
DataMind three-scope & \tbd & \tbd & \tbd & \tbd & \tbd \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[t]
\centering\footnotesize
\caption{Tool-dispatch SQL policy diagnostic. Ask-user decisions count as
...
\midrule
Direct execution & \tbd & \tbd & \tbd \\
Keyword guard & \tbd & \tbd & \tbd \\
DB-GPT & \tbd & \tbd & \tbd \\
DataMind hooks & \tbd & \tbd & \tbd \\
\bottomrule
\end{tabular}
\end{table}
...
accuracy result. DB-GPT and RAGFlow are therefore retained as external
baselines only on task subsets that their released systems support; quantitative
comparisons use the same model, inputs, prompts, and output budget. A partial
or unsupported capability is not treated as a failed answer on a workload that
the system was not designed to serve.
