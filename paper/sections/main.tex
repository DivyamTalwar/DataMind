...
The evaluation follows one artifact lineage rather than four independent
benchmarks. We first build a workspace and publish a versioned snapshot; the
Serving Agent answers WorkSurface-Bench requests against that snapshot; we then
mutate the source or fail a provider while the same lineage is active; finally,
we replay the serving workload at controlled concurrency. This makes the
causal dependencies explicit: RQ2 produces the artifact consumed by RQ1, RQ3
checks whether that artifact remains correct through its lifecycle, and RQ4
measures the cost of serving it.

\begin{itemize}
  \item \textbf{RQ1}: On a fixed published snapshot, does surface-aware
  serving improve route, evidence, answer quality, and efficiency?
  \item \textbf{RQ2}: What does it cost to materialize the surfaces, and when
  is the build cost amortized by subsequent queries?
  \item \textbf{RQ3}: Under source updates and component failures, are revision,
  provenance, profile isolation, and safety boundaries preserved?
  \item \textbf{RQ4}: What latency, throughput, memory, and hook overhead does
  managed serving add under concurrency?
\end{itemize}
...
Every result records the workspace checksum, profile, published snapshot, and
revision consumed by the task. The build, lifecycle, and runtime experiments
reuse these identifiers so that a quality result cannot silently come from a
...
\subsection{RQ3: Lifecycle correctness under updates and failures}
\label{sec:eval-rq3}

RQ3 tests the claim that a workspace data plane has a lifecycle, rather than
only a set of retrieval endpoints. We compare two executions over the same raw
workspace and serving task trace. A \emph{direct mutable pipeline} writes
candidate data directly into live backends and lets readers observe whatever
state is present. \emph{DataMind-managed} builds a candidate, validates it,
publishes a snapshot, and pins each request to that published revision; profile
and destructive-operation guards return structured failures when the required
boundary cannot be maintained.

We inject source-update/read overlap, MinerU or parser and embedding failures,
database and SQL timeout/connection/schema failures, graph unavailability, and
corrupted artifacts. We also replay the existing profile-memory isolation and
SQL-policy probes as diagnostics of the same boundary. The primary measures
are stale or mixed reads, correct-revision rate, auditable-answer rate,
structured-error accuracy, fallback/block correctness, recovery time, memory
leakage, and unsafe execution. An answer is counted as recovered only when it
meets the task requirement, cites valid evidence, and names the correct
revision; guessing a plausible answer is not recovery.

\begin{table*}[t]
\centering\footnotesize
\setlength{\tabcolsep}{6pt}
\caption{RQ3 lifecycle correctness after updates and injected surface faults.}
\label{tab:rq3-lifecycle}
\scriptsize
\setlength{\tabcolsep}{4pt}
\resizebox{0.90\textwidth}{!}{%
\begin{tabular}{@{}llrrrrrr@{}}
...
\midrule
Source update/read overlap & Direct mutable & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Source update/read overlap & DataMind-full & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
...
Active candidate read & Direct mutable & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
Active candidate read & DataMind-full & \tbd & \tbd & \tbd & \tbd & \tbd & \tbd \\
...
\subsection{RQ4: Runtime overhead and concurrency}
\label{sec:eval-rq4}

RQ4 uses the fixed snapshot and serving workload from RQ1. The only controlled
change is the serving path: \emph{DataMind no-hooks} retains the same model,
surface contents, prompt, and tool schema while disabling policy and audit
hooks; \emph{DataMind full} enables the default hooks, evidence metadata,
snapshot selection, and audit path. An external system is included only when
its model, data, task set, and decoding budget are matched.
...
latency, QPS, peak RSS, tool calls, tokens, and a model/tool/hooks latency
breakdown. This isolates the runtime cost of DataMind's management semantics;
it does not reinterpret the old single-concurrency pilot as a quality result.

\begin{table*}[t]
\centering\footnotesize
\setlength{\tabcolsep}{6pt}
\caption{RQ4 end-to-end latency, throughput, and memory under concurrency.
...
its input checksum and published snapshot; serving records accessed surfaces,
evidence, revision, calls, tokens, and latency; lifecycle runs additionally
record the injected event and the expected block or fallback. Thus the tables
