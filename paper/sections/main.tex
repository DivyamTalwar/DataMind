...
into data surfaces: parsed and indexed documents, relational projections,
dependency graphs, and persistent memory. A \emph{Serving Agent} consumes a
...
operations propagate those receipts into the answer trace. A new build is
prepared privately and becomes visible only when its surface artifacts pass
validation and publication. This gives the system an explicit update boundary
without requiring every physical backend to implement the same storage model.
...
\begin{itemize}
  ...
  materializes, publishes, routes, updates, and recovers document, relational,
  graph, and memory surfaces for tool-using agents.
...
\Description{DataMind architecture showing the Build Agent materializing workspace artifacts into document, database, graph, and memory surfaces, and the Serving Agent routing requests over a published profile snapshot.}
\end{figure*}
...
can be traced to the files that induced its edges. Memory records are written
with an explicit global, profile, or session scope. The Build Agent is allowed
to produce only the surfaces selected by the profile configuration; it does not
assume that every workspace needs every representation.
...
to decide which arguments are legal, source and revision metadata to avoid
mixing incompatible views, and cost/freshness metadata to rank alternatives.
...
When a preferred operation fails, the Serving Agent classifies the failure
before choosing a fallback. An empty retrieval result is evidence that the
surface returned no matching item; an unavailable graph store is an execution
failure. A fallback is permitted only when another surface's contract can
provide the required evidence. The trace records the failed operation, the
fallback, and the receipts used by the final answer.
...
code, and PDF. Office documents (DOC/DOCX and PPT/PPTX) can be materialized to
PDF for a common layout-aware parsing path; tabular files (CSV/TSV and
XLS/XLSX) are loaded into typed relations. The original file remains the
...
The static serving ablation removes surface manifests and evidence receipts one
at a time. Snapshot publication and bounded fallback are evaluated in the
dynamic update and fault-injection experiments, respectively, because a static
benchmark with one valid revision and no injected failures cannot identify
their effects. If the full system improves only answer quality but increases
unnecessary exploration, the additional tool and token measurements make that
trade-off explicit. If the same quality is achieved by the simple manifest
router, the result identifies which parts of DataMind still need a stronger
planner rather than attributing all gains to the data-plane abstraction.
...
surfaces, parsers, and physical backends as DataMind but omits manifest-driven
validation, receipts, snapshot publication, and revision management. Build metrics include wall
clock time, parser and embedding calls, storage, source coverage, provenance
...
without snapshot publication, and the complete system.
...
We inject parser failures, embedding-service failures, empty document results,
...
whether its answer is correct, whether the evidence remains auditable, how
long recovery takes, and how much data is rebuilt. We compare retry-only,
fixed-path, DataMind without recovery metadata, and DataMind full.

The fault matrix distinguishes unavailable data from empty data. A fallback is
counted as successful only if it satisfies the task's evidence requirement; a
plausible answer without a valid receipt is a failure. This prevents the
recovery experiment from rewarding a model for guessing after a source outage.
