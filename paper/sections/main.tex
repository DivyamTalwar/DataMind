...
This diagnostic table isolates the effect of ingestion components. It compares chunkers and embedders so that retrieval differences are not misattributed to the overall build strategy.

\begin{table}[t]
\centering\footnotesize
\caption{Pilot ingestion component sensitivity from the earlier report on its
MS MARCO subset. These values must be rerun before being used as a primary
WorkSurface-Build result.}
\label{tab:rq2-ingest}
\begin{tabular}{@{}lllrrr@{}}
\toprule
System & Chunker & Embedder & NDCG@10$\uparrow$ & Recall@10$\uparrow$ & MRR$\uparrow$ \\
\midrule
...
hooks, evidence bookkeeping, and audit path. DB-GPT and RAGFlow are required on their matched database or document
subsets with the same model, data, questions, and output budget; unsupported
surface combinations are marked N/A rather than omitted silently.

We run 60 or 200 tasks at concurrency 1, 5, 20, 50, and 100, with at least
three repetitions per condition. We report errors, p50/p95/p99 end-to-end
latency, QPS, peak RSS, tool calls, tokens, and model/tool/hook latency
breakdowns. This isolates the cost of DataMind's serving and management path
without confusing it with answer-quality differences.
